...
struct ceasarEncode {
    ...
    //math: (char - shift + 26) % 26
    static func decrypt(shift: Int, cipherText: String) -> String {
        ...
        let decryptedChars = chars.map { char -> Character in
            ...
            //(cipher - base - shift + 26) % 26 + base
            //we add 26 before modulo to handle negative results from subtraction
            //using int for calculation to avoid underflow before adding 26
            let shiftedValue = Int(ascii) - Int(base) - shift + 26
            let newAscii = base + UInt8(shiftedValue % 26)
            
            //this is needed to return the correct char as it checks if the value is valid before
            return Character(UnicodeScalar(newAscii))
        }
        
        return String(decryptedChars)
    }
    
    //existing encrypt function kept for reference
